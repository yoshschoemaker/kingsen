    ...
    private ArrayList<Card> addCardRules(ArrayList<Card> cards) {
        ...
        for (Card card : cards) {
            if (card.isNumberCard()) {
                switch (card.getNumber()) {
                    ...
                    case 10:
                        ...
                        card.getRule().setDescription("Everyone hold up 3 fingers. The person who drew this card says a sentence starting with ‘Never have I ever..’ and complete with something he/she hasn’t done.  Whoever has done it, lowers one finger. First person to lowers all fingers gets the penalty.");
                        break;
                }
            }
            ...
            if (card.getFace() == Face.JACK) {
                card.getProperties().setPlayable(true);
                card.getRule().setRuleTitle("QUIZZMASTER!!1");
